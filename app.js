const { useState, useEffect, useMemo, useCallback } = React;
                ...
                const CORES_PARTIDO = { 'PMDB': '#94a3b8', 'PSDB': '#60a5fa', 'PT': '#f87171', 'MDB': '#fbbf24', 'PL': '#34d399' };
                const CORES_CAT = { 'Economia': '#ea580c', 'Economia EUA': '#0a580c', 'Impostos e Consumo': '#ea283c', 'Institucional': '#0a58ac', 'Política': '#7a88ac', 'Internacional': '#7a28bc', 'Social': '#aa58ac', 'Bolsas': '#6a580c', 'Moedas': '#3aa82c', 'Emprego e Renda': '#16a34a', 'Desenvolvimento': '#dc2626', 'Saúde': '#0891b2', 'Educação': '#db2777', 'Meio Ambiente': '#16234a', 'Dívida': '#475569', 'Ciência e Tecnologia': '#0d9488', 'Corrupção': '#ca8a04', 'Segurança': '#9f1239', 'Commodities': '#9f12a9' };
                    ...
                    <div className="legend-mandatos-mobile">
                      {PRES_ANOS.map(p => (
                        <div key={p.nome} style={{ display: "flex", alignItems: "center", gap: 4 }}>
                          <span style={{ width: 12, height: 8, backgroundColor: CORES_PARTIDO[p.partido], opacity: 0.3, borderRadius: 2, border: `1px solid ${CORES_PARTIDO[p.partido]}` }}></span>
                          <span>{p.nome}</span>
                        </div>
                      ))}
                    </div>
                      ...
                      <div style={{ display: "flex", flex: 1, overflow: "hidden", minHeight: 0 }}>
                        ...
                        <div className={"sidebar sidebar-anual" + (menuAberto ? " aberto" : "")} style={{ width: 320, flexShrink: 0, borderRight: "1px solid #e1e4e8", padding: "24px 16px 150px 16px", display: "flex", flexDirection: "column", gap: 16, background: "#fff", overflowY: "auto", position: "relative", zIndex: 10 }}>
                          ...
                          {Object.entries(cats).map(([cat, keys]) => (
                            <div key={cat}>
                              <div className="cat-label" style={{ color: CORES_CAT[cat], fontSize: 13, textTransform: "uppercase", letterSpacing: "0.5px", marginBottom: 8 }}>{cat} ({keys.length})</div>
                              <div className="ind-grid" style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                                {keys.map(k => (
                                  <button key={k} type="button" className={`ind-btn${selecionados.includes(k) ? ' ativo' : ''}`} style={{ flex: "1 1 100%", minWidth: 0, padding: "6px 8px", fontSize: 12, display: "flex", alignItems: "center", gap: 6, justifyContent: "flex-start", transition: "all 0.2s", ...(selecionados.includes(k) ? { backgroundColor: "#f8fafc", color: "#000", border: `1px solid ${CORES_CAT[cat]}`, boxShadow: `inset 3px 0 0 ${CORES_CAT[cat]}` } : { border: "1px solid #e1e4e8" }) }} onClick={e => { e.preventDefault(); toggle(k); }}>
                                    <span style={{ width: 8, height: 8, borderRadius: "50%", backgroundColor: CORES_CAT[cat], flexShrink: 0 }}></span>
                          ...
                          <div style={{ padding: "16px 24px 150px 24px", display: "flex", flexDirection: "column", gap: 16 }}>
                            {selecionados.length === 0 ? (
                              ...
                              selecionados.map(k => {
                                ...
                                return <GraficoAnual key={k} dados={dadosFiltrados} info={meta[k]} cor={CORES_CAT[meta[k].cat] || '#79c0ff'} anoIni={anoIni} anoFim={anoFim} />;
                              })
                            )}
                          </div>
                        </div>
                        ...
                        <ResponsiveContainer width="100%" height={240}>
                          <LineChart data={dadosComEventos} margin={{ top: 10, right: isMobile ? 8 : 16, bottom: 10, left: isMobile ? -10 : 8 }}>
                            {PRES_ANOS.filter(p => Math.min(anoFim, p.fim) > Math.max(anoIni, p.ini)).flatMap((p, i) => {
                              ...
                              const x2 = p.fim - 0.5;
                              return [
                                <ReferenceArea key={`area-${i}`} x1={x1} x2={x2} fill={CORES_PARTIDO[p.partido]} fillOpacity={0.08} ifOverflow="hidden" />,
                                p.ini >= anoIni ? <ReferenceLine key={`line-${i}`} x={x1} stroke="#000" strokeOpacity={0.2} strokeDasharray="3 3" ifOverflow="hidden" /> : null,
                                <ReferenceArea key={`label-${i}`} x1={x1} x2={x2} fill="transparent" ifOverflow="hidden" label={{ value: p.nome, position: 'insideBottom', fill: CORES_PARTIDO[p.partido], fontSize: 11, fontWeight: "bold", dy: -5 }} />
                              ];
                            })}
